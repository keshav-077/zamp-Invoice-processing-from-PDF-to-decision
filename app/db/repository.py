...
def save_run(result: PipelineResult, company_id: str = "DEFAULT") -> None:
    ...
    conn.execute(
        ...
        (
            ...
            to_native_number(result.processing_time_seconds),
            json.dumps(json_safe([p.model_dump(mode="json") for p in result.pages])),
            model_dump_json_safe(result.extraction),
            model_dump_json_safe(result.verification),
            model_dump_json_safe(result.arithmetic),
            model_dump_json_safe(result.reconciliation),
            to_native_number(result.document_quality_score),
            result.decision,
            json.dumps(json_safe(result.decision_explanation)),
            result.retry_count,
            result.error_details,
            result.original_file_path,
            model_dump_json_safe(result.stage2_result),
            result.stage2_status,
            model_dump_json_safe(result.stage3_result),
            result.stage3_status,
            model_dump_json_safe(result.stage4_result),
            result.stage4_status,
            result.stage4_decision,
            model_dump_json_safe(result.stage5_result),
            result.stage5_status,
            result.stage5_explanation_id,
            company_id,
            model_dump_json_safe(result.evidence_profile),
